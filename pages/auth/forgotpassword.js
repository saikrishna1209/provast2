...
export default function Example() {
  ...
  const handleSubmitPassword = async (e) => {
    ...
    if (password === confirmPassword) {
      ...
      const {
        data: { message },
      } = await axios.put(`/api/auth/forgotpassword/?email=${email}&password=${password}`);
      setLoading(false);
                ...
                <div>
                  ...
                  <div className='mt-1'>
                    <input
                      ...
                      className={`${
                        state !== "password" ? "bg-gray-100 cursor-not-allowed" : ""
                      } appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 sm:text-sm`}
                    />
                  </div>
                </div>
...
export const getServerSideProps = async function ({ req, res }) {
  ...
  if (user?.detailsAvailable && user.category === "student") {
    return {
      redirect: {
