...
import { languageoptions } from '../../../lib/helper';
import { ExtraOption } from './Shared/ExtraOption';
...
export const LanguageForm = () => {
  ...
  const { resume, languages, setLanguages, debounceUpdateResume } =
    useResumeContext();
  ...
  languages.forEach((skill) => currentSkills.add(skill.name));

  const [skillOptions, setSkillOptions] = useState(languageoptions);
  ...
  const handleSubmit = (e) => {
    ...
    if (isEdit) {
      newstate = newstate.map((x) => {
        if (x._id === editId) return { ...option, fluency: selected.name };
        return x;
      });
    } else {
  ...
  useEffect(() => {
    if (isEdit) setOption(languages.filter((x) => x._id === editId)[0]);
  }, [isEdit, editId, languages]);
  return (
